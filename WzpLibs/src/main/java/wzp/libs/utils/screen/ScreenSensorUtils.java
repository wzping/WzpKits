 ...
 * 屏幕传感器(监听是否横竖屏)
 */
public final class ScreenSensorUtils {

	// 日志 TAG
	private final String TAG = ScreenSensorUtils.class.getSimpleName();
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
				case CHANGE_ORIENTATION_WHAT:
					...
					// -
//					LogUtils.d(TAG,"当前角度: " + rotation);
					// 判断角度
					if (rotation > 45 && rotation < 135) { // 横屏 - 屏幕对着别人
//						LogUtils.d(TAG, "切换成横屏 - 屏幕对着自己");
						// -
						if (isPortrait) {
							isPortrait = false;
							if(handler != null){
...
//						LogUtils.d(TAG,"其他角度: " + rotation);
	...
	public void start(Context context, Handler handler) {
		isAllowChange = true;
		try {
//			LogUtils.d(TAG, "start orientation listener.");
			// 初始化操作
	...
	public void stop() {
		isAllowChange = false;
//		LogUtils.d(TAG, "stop orientation listener.");
		try {
